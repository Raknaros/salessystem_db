...
def load_cotizaciones(archivo):
    ...
    for col in columns_to_round:
        cotizaciones[col] = cotizaciones[col].apply(lambda x: round(float(x), 3) if isinstance(x, str) and x.replace('.', '', 1).replace('-', '', 1).isdigit() else round(x, 3) if pd.notna(x) else x)
